   ...
   const getDriver= (details) => {
      ...
      .then(response => {
        if(response.data.length === 0){
          ...
          const dDetails = response.data;
          setDriverDetails(dDetails);
          console.log(driverDetails.driver_details[0].names);
          const driverPosition = {
            latitude: driverDetails.trip_details[0].deviceLatitude || 0,
            longitude: driverDetails.trip_details[0].deviceLongitude || 0,
          };

          const pickUpPosition = {
            latitude: driverDetails.trip_details[0].pickUpPointLat || 0,
            longitude: driverDetails.trip_details[0].PickUpPoitLong || 0,
          }
          const destination = {
            latitude: parseFloat(driverDetails.trip_details[0].destinationLatitude) || 0,
            longitude: parseFloat(driverDetails.trip_details[0].destinationLongititude) || 0,
          }
          // console.log(parseFloat(response.data[0].PickUpPoitLong));
          setMarker2({latitude:parseFloat(driverDetails.trip_details[0].destinationLatitude),longitude:parseFloat(driverDetails.trip_details[0].destinationLongititude)});
           setMarker1({latitude:parseFloat(driverDetails.trip_details[0].pickUpPointLat),longitude:parseFloat(driverDetails.trip_details[0].PickUpPoitLong)});
        }
      })
      .catch(error => {
        console.log(error);
